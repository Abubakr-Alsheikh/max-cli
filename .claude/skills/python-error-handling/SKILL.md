...
- Raise `MaxError` subclasses from `max_cli.common.exceptions` (`ValidationError`, `ResourceNotFoundError`, `ProcessingError`, `NetworkError`, `AIError`, `ConfigurationError`), not bare builtins, across engine boundaries.
- No `except Exception` in engines; broad catches only at the interface top level and must call `log_error`.
- No `print` anywhere in core. Progress goes through `EventEmitter` (`max_cli.common.events`), not callbacks.
- HTTP library is `requests` (lazy-imported), not httpx. Stdlib `logging` does not accept arbitrary kwargs.
