...
- Use `unittest.mock` + fixtures in `tests/conftest.py`. freezegun, hypothesis, sqlalchemy, and pytest-mock are **not** project dependencies; ask before adding any.
- Keep the existing flat layout: `tests/test_core_*.py`, `tests/test_cli_*.py`, `tests/interface/tui/`. No `test_unit/`/`test_integration/` split.
- No `print` in fixtures; don't change pytest `addopts`/coverage config without asking.
- Project-specific rules live in the `max-testing` skill; it wins on conflict.
