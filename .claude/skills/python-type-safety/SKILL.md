...
- Type checker is mypy with `mypy.ini`; do not switch to `--strict` or change `python_version` in config without asking.
- Every `# type: ignore[code]` needs a short reason comment after it (AGENTS.md section 10; enforced by hook).
- `TYPE_CHECKING` imports use quoted annotations: `"ClassName"`.
