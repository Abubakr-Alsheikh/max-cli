...
- Run `ruff` directly (installed via `pip install -e .[dev]`), not `uv run`/`uvx`.
- Never use `--unsafe-fixes` or `--watch` unattended. The PostToolUse hook already runs `ruff check --fix` + `ruff format` on edited files.
- Ruff config lives in `pyproject.toml` `[tool.ruff]`; changing it is an "ask first" edit.
