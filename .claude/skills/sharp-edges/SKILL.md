...
## Project overrides (Max CLI)

These rules win over anything below. Source: `AGENTS.md`.

- Only Python references are vendored (`lang-python.md`, `language-specific.md`, `config-patterns.md`). The upstream `sharp-edges-analyzer` agent is not installed.
- High-value targets in this repo: `config.py` (Pydantic settings defaults), subprocess/FFmpeg argument building, AI-JSON-to-path handling (`smart_sort`), tar/zip extraction, plugin loading from `./plugins`, JSON state files.
...
- **Python footguns**: See [references/lang-python.md](references/lang-python.md)

Other upstream references (crypto, auth, case studies, non-Python languages) are omitted: https://github.com/trailofbits/skills/tree/main/plugins/sharp-edges
