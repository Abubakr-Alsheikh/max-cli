...
- Test runner is `pytest` (not `npm test`). Conventions: `max-testing` skill.
- Mocks: AGENTS.md section 13 **requires** mocking FFmpeg (`subprocess.run`, `shutil.which`), network, and AI APIs. "No mocks" applies only to pure-Python logic we own.
- Code written before its test: rewrite it test-first when practical. Don't silently delete the user's uncommitted work; ask first.
- `superpowers:writing-skills` is not installed; ignore that reference.
