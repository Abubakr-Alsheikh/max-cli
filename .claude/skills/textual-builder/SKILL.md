...
description: Build and test Textual TUI code, including the Max CLI `max dashboard` (src/max_cli/interface/tui). Use when writing or fixing Textual widgets, screens, TCSS, workers, or Pilot tests, or when the user mentions Textual, TUI, dashboard, or terminal UI. Includes comprehensive reference documentation, card game starter template, and styling guides.
---
...
```bash
pip install -e .[tui]   # from the max-cli repo root
```

Installed in this project: Textual 8.2.7 (pyproject: `textual>=0.48`).
...
- `workers-and-testing.md` - Thread/async workers and Pilot tests (verified on 8.2.7)
