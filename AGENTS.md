...
- **Command Catalog Pattern** (roadmap Step 2, `PLANS/active/command-catalog.md`; ported groups: `video`, `images`, `pdf`, and `grab download`, whose CLI calls the operation but keeps its own flags):
...
- **TUI Dashboard Pattern**:
  ...
  - Download page: `widgets/download_panel.py` calls `core/operations/grab.py`. It shows a preview (`grab.probe`), quality choices, Simple or Advanced mode, and one `DownloadRow` per download with a real Cancel (`should_cancel`). `GRAB_MAX_CONCURRENT` limits parallel downloads. `interface/tui/ui_prefs.py` remembers small page choices
